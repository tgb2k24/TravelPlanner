...
type CreateTripRouteProp = RouteProp<RootStackParamList, 'Create'>;
...
const earthSvg = `
<svg fill="#000000" viewBox="0 0 23.051 23.051" xmlns="http://www.w3.org/2000/svg">
...
const CreateTrip: React.FC = () => {
    ...
    const handleCreateTrip = async (): Promise<void> => {
        ...
        const tripData = {
            ...
            startDay,
            endDay,
            timezone: 'Asia/Kolkata', // Bengaluru, India
            background,
            host: userId,
        };
        ...
        try {
            ...
            // Navigate to TripScreen with the created trip data
    ...
    /* ----------------------------------
       Date Helpers
    ----------------------------------- */
    const setDates = (dates: PickerDateRange): void => {
        if (dates.startDate) {
            const start = moment(dates.startDate);
            setStartDate(start);
            setStartDay(start.format('dddd'));
        }
        if (dates.endDate) {
            const end = moment(dates.endDate);
            setEndDate(end);
            setEndDay(end.format('dddd'));
        }
